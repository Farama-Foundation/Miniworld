...
full_map_shape = (3, 900, 900)
local_map_shape = (3, 200, 200)
num_trans = 0
cur_idx = 0
...
input_map = np.zeros(shape=(args.buffer_size,) + full_map_shape, dtype=np.float32)
output_map = np.zeros(shape=(args.buffer_size,) + local_map_shape, dtype=np.float32)
...
def gen_data(num_episodes=1):
    ...
    for i in range(num_episodes):
        ...
        # Full overhead map
        full_map = make_var(np.zeros(shape=full_map_shape)).unsqueeze(0)
        ...
        for step_idx in range(max_steps):
            ...
            input_map[cur_idx] = full_map.cpu().numpy()
            ...
            x, _, z = env.agent.pos
            i = math.floor(200 + (200/6) * x)
            j = math.floor(200 + (200/6) * z)
            #print(env.agent.pos)
            #print(x, i)
            #print(z, j)
            #print(full_map.shape)
            in_local_map = full_map[:, :, i:(i+200), j:(j+200)]
            ...
            with torch.no_grad():
                ...
                out_local_map = model(obs, in_local_map, pos)


            full_map[:, :, i:(i+200), j:(j+200)] += out_local_map
...
for i in range(1000000):
    ...
    x = batch_pos[:, 0]
    z = batch_pos[:, 2]
    i = torch.floor(200 + (200/6) * x).long()
    j = torch.floor(200 + (200/6) * z).long()

    print(i.size())

    batch_local_in = batch_in_map[:, :, i:(i+200), j:(j+200)]
